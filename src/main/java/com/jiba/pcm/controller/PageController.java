...
//    }
    ...
    @GetMapping("/errors")
    public String loginError(@RequestParam(value = "disabled", required = false) String disabled, Model model) {
        if ("true".equalsIgnoreCase(disabled)) {
            model.addAttribute("disabledAccount", "Your email is not verified. Please check your inbox to verify your email.");
        } else {
            model.addAttribute("errorPage", "🙃 you have jumped to the wrong page.");
        }
        return "errors";
    }

    @GetMapping("/mail-verification")
    public String mailError(@RequestParam(value = "verified", required = false) String verified, Model model) {
        if ("true".equalsIgnoreCase(verified)) {
            model.addAttribute("verified", "Your email is verified 🤩. Now you can login by clicking on -> GO TO LOGIN PAGE button.");
        } else {
